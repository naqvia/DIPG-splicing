...
my (@non_clk1_samples,@clk1_samples,@splice_id, @nonDMG_clk1_samples, @nonDMG_non_clk1_samples);
my (%tumor_IJCs, %splice_id_count);

my %clk1_samples_filter;
my %non_clk1_samples_filter;
my %clk1_HGGs_samples_filter;
...
open(INP,"CLK_samples.HGGs.txt") || die("Cannot Open File");
while(<INP>)
{
  chomp;
  my $sample = $_;
  #print "SAMPLE".$sample."SAMPL\n";
  $clk1_HGGs_samples_filter{$sample} = $sample;
}
close(INP);
...
while(<FIL>)
{
  ...
  if($_=~/control\-BR/)
  {
    ...
    if($clk1_HGGs_samples_filter{$sample})
    {
      push @nonDMG_clk1_samples,$sample;
    }
    else{
      push @nonDMG_non_clk1_samples,$sample;
    }
  ...
  while(<SE_FIL>)
  {
    ...
    if($_=~/^\d+/)
    {
     ...
     # print "*".$sample,"\t",$splice_id,"\t",$tumor_IJC, "\t",$tumor_SJC,"\t",$inc_from_len,"\t",$skip_from_len,"\n";
      ...
      $tumor_IJCs{$splice_id}{$sample} = $thr_diff;
  ...
  #print join "\t", @clk1_samples,"***\n";
   ...
   foreach my $sample(@nonDMG_non_clk1_samples)
   {
     print ",NonHGG";
     print $sample;
   }

   foreach my $sample(@nonDMG_clk1_samples)
   {
     print ",CLKHGG";
     print $sample;
   }
