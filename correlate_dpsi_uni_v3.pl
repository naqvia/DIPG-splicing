...
## baf complex: baf_compl_dpsi_patients.txt
## universal universal_dpsi_patients_v2
## recurrent and isoform switching: recurrent_switching_dpsi_patients.txt##outframe_mexons_dpsi_patients.txt
## utr 3 :ms3utrgenes_w_mirtarget.ts.dpsi_patients
open(FIL,"/Users/naqvia/Desktop/DIPG/ms3utrgenes_w_mirtarget.ts.dpsi_patients.txt") || die("Cannot Open File");
while(<FIL>){
    chomp;
    my ($bs_id,$lsv,$psi) = split "\t";
    my (@events) = split/\;/,$psi;
    my ($gene,$lsv_v2) = split/\_/,$lsv;
    #print "psi_event:".$events[0],"\n";
    my $lsv = $lsv_v2;
    #print "*".$lsv,"\n";
    #next unless ($lsv=~/ENSG00000133884/);
    $majiq_psi{$bs_id}{$lsv} = $events[0];
...
my @lsvs_uniq = do { my %seen; grep { !$seen{$_}++ } @lsvs };
  ...
  {
    next unless($ens_to_gene{$gene}=~/\w/);
    my $gene_name = $ens_to_gene{$gene};
    print $gene."_".$gene_name."_expr,";
  }
